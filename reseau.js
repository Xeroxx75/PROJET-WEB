...
function chargerMoi() {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                ...
                // Vérifier si les données de l'utilisateur (moi) sont disponibles dans la réponse
                if (donnees.moi) {
                    var moi = donnees.moi[0]; // Première (et unique) entrée dans le tableau "moi"

                    // Récupérer les informations de l'utilisateur (moi)
                    var nom = moi.nom;
                    var prenom = moi.prenom;
                    var photoProfil = moi.photo_profil;
                    var description = moi.description;

                    // Créer les éléments HTML pour afficher les informations de l'utilisateur (moi)
                    var profilDiv = document.createElement('div');
                    profilDiv.classList.add('profil-moi');
                    ...
                    a.addEventListener('click', function() {
                        ...
                        for (var i = 0; i < elements.length; i++) {
                            if (elements[i].getAttribute('href') === 'profil') {
                                vousLink = elements[i];
                                break;
                    ...
                    // Ajouter les éléments au conteneur
                    profilDiv.appendChild(h3);
                    profilDiv.appendChild(a); 
                    a.appendChild(img);
                    profilDiv.appendChild(p);

                    // Ajouter le conteneur à la section d'abonnements
                    var abonnementsSection = document.getElementById('vous');
                    abonnementsSection.appendChild(profilDiv);
                } else {
                    console.error("Les données de l'utilisateur (moi) sont indisponibles dans la réponse.");
                }
            } else {
                console.error("Erreur de chargement des données : " + xhr.status);
            }
    ...
    xhr.open("GET", "reseau_moi.php", true);
    xhr.send();
}
